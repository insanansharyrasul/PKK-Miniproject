...
= V. Hasil dan Diskusi
Eksperimen dijalankan pada 27 lokasi Jawa Barat. Modul _auto-tuning_ memilih _preset_ Exploitation-Heavy dengan parameter $w=0.4$, $c_1=1.0$, $c_2=2.0$, 15 partikel, _mutation rate_ 0.05, batas 100 iterasi, dan _seed_ 42 untuk reprodusibilitas.
...
#align(center)[
  #block(width: 100%)[
    ...
    #table(
      ...
      [Greedy NN], [891.94], [0.00 (Ref)], [0.00%], [2.5289 ms],
      [Memetic PSO], [784.61], [-107.33], [12.03%], [15.73 s],
      table.hline(stroke: 1pt),
    )
  ]
]

_Memetic PSO_ berhasil memotong panjang rute sebesar 107.33 km (peningkatan efisiensi 12.03%) dibandingkan _baseline Greedy_. Peningkatan ini terjadi karena _Greedy_ hanya mengambil keputusan lokal terbaik tanpa mempertimbangkan dampak jangka panjang rute, sehingga sering menyisakan kota-kota terjauh di akhir pencarian yang mengakibatkan lompatan rute balik sangat jauh. _Memetic PSO_ mampu mematangkan rute secara global melalui kolaborasi antar partikel [1].
...
_Greedy_ menyelesaikan pencarian dalam 2.5289 ms karena kompleksitasnya $O(N^2)$, sedangkan _Memetic PSO_ membutuhkan 15.73 s. Selisih waktu ini merupakan _trade-off_ yang dapat diterima mengingat keuntungan penghematan rute 12.03%. Sebagian besar waktu PSO dihabiskan oleh operasi _2-opt_ periodik setiap 5 iterasi; _Early Stopping_ menghentikan _loop_ pada iterasi ke-80 dari 100 iterasi maksimum. Perbandingan waktu komputasi kedua metode divisualisasikan pada @fig-kinerja (panel tengah).

== C. Analisis Konvergensi dan Diversitas _Swarm_
Kurva konvergensi pada @fig-konvergensi menunjukkan bahwa rute $G_("best")$ langsung dimulai dari jarak _Greedy_ (891.94 km) karena _heuristic seeding_ pada partikel ke-0. Operasi _2-opt_ periodik secara bertahap mengeliminasi persilangan jalur, sehingga jarak turun progresif hingga 784.61 km ketika _early stopping_ terpicu pada iterasi ke-80.
